...
void Arvore::imprimeEDR(No* atual){
	if(atual){
	
        imprimeEDR(atual->esquerda);
        imprimeEDR(atual->direita);
        cout<<"["<<atual->valor<<"]"<<endl;
		}
	}
...
void Arvore::balanceamento(No* no){
	while(no){
		if(FB(no) >= 2 && FB(no->direita) >= 0){
			LeftRotate(no);
		} 
		else if(FB(no) <= -2 && FB(no->esquerda) <= 0){
			RightRotate(no);
		} 
		else if(FB(no) >= 2 && FB(no->direita) < 0){
			RightRotate(no->direita);
			LeftRotate(no);
		} 
		else if(FB(no) <= -2 && FB(no->esquerda) > 0){
			LeftRotate(no->esquerda);
			RightRotate(no);
		}
		no = no->pai;
	}
